...
impl JitCompiler {
    ...
    fn execute_file(&mut self, file_path: &PathBuf) -> Result<(), String> {
        ...
        match self.interpreter.evaluate(&ast) {
            Ok(result) => {
                ...
                // Update cache
                self.file_cache.insert(file_path.clone(), (modified, content));
    ...
    fn start_hot_reload(&mut self, file_path: PathBuf) -> Result<(), String> {
        ...
        let watch_dir = file_path.parent().unwrap_or_else(|| std::path::Path::new("."));
                ...
                Ok(event) => {
                    ...
                    match event {
                        ...
                        notify::DebouncedEvent::Create(path) => {
                            if path.extension().and_then(|s: &std::ffi::OsStr| s.to_str()) == Some("j") {
